...
// una "huella" de 64 bits de la imagen completa y la compara contra las
...
/**
 * Calcula el pHash de 64 bits a partir de un ImageData de 32x32.
 ...
 * dibujar el frame de camara (con crop, etc.) antes de hashear, y al
 * diccionario dibujar imagenes pre-cargadas antes de hashearlas.
 *
 * @param {Uint8ClampedArray} rgba datos RGBA de un ImageData 32x32
 * @returns {Uint8Array} 8 bytes (64 bits) - el hash
 */
export function hashFromImageData(rgba) {
  ...
  const hash = new Uint8Array(8); // 64 bits en 8 bytes
...
/**
 * Calcula el pHash de 64 bits de una imagen/video dibujandola en un canvas
 * chico (32x32) y aplicando DCT-II 2D. Atajo para cuando el llamador no
 * necesita hacer crop ni transformaciones antes de hashear.
 *
...
/**
 * Distancia de Hamming entre dos hashes de 8 bytes (cuantos bits difieren).
 * @param {Uint8Array} a
 * @param {Uint8Array} b
 * @returns {number} 0..64
 */
